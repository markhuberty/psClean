...
share of all retrieved matches.\footnote{This method for computing
  ...
  were assigned to a common group, compared with how many should have
  been so assigned. This is not the only
...
\begin{landscape}
  \begin{table}[t]
    ...
    \begin{tabular}{l|c|c|c||c|c|c|c}
      ...
      \hline
      1 & 5 & 1 & 1 &
      \multirow{5}{*}{$P_{id} = \frac{3}{5}$} &
      \multirow{5}{*}{$P_{patent} = \frac{5 + 1 + 7}{18}$} &     
      \multirow{5}{*}{$R_{id} = \frac{4}{5}$} & 
      \multirow{5}{*}{$R_{patent} = \frac{5 + 1 + 7 + 2}{18}$}\\
      \cline{1-4}
      2 & 1 & 1 & 1 & & & &\\
      \cline{1-4}
      3 & 3 & 1 & 2 & & & &\\
    ...
    \caption{Stylized example for calculating precision and recall values for \texttt{dedupe} in
      reference to the Leuven dataset. For ID precision note that we
      correctly assign 3 PATSTAT IDs to unique \texttt{dedupe} IDs:
      1=2, 2=1, and 4=2. For ID recall, we assign 4 IDs to unique
      groups (1=1, 2=1, 4=2, and 5=2). The same analysis holds for
      patent-level data. As we should expect, in this instance
      precision suffers (due to clumping 4 and 5 together, and
      clumping 3 with 1 and 2), compared with recall.}
