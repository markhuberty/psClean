...
iso_codes = iso_codes.dropna(axis=0)

## Walk across the files and geocode non-blank addresses
datadir = './data/cleaned_data'
sample_addresses = 'sample_country_addresses.csv'

df = pd.read_csv(datadir + '/' + sample_addresses)
...
for addr, country in it.izip(df['person_address'], df['person_ctry_code']):
    if isinstance(addr, str) and len(addr) > 0:
        if country in iso_codes['country_code'].values:
            country_name = iso_codes['country_name'][iso_codes['country_code'] == country].values[0]
        else:
            country_name = ''

        latlng = geocode_address(addr, country_name, base_url)
    else:
        latlng = (None, None)
...
df.to_csv(datadir + '/sample_latlng.csv')
